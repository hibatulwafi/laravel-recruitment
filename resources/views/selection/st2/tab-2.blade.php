<div class="row">
  <!-- <div class="col-6">
    <h4 class="card-title">Loker Sebelumnya</h4>
    <p>Riwayat loker sebelumnya</p>
  </div>
  <div class="col-6">
      ...
      <table id="datatable" class="table table-bordered table-hover" data-toggle="data-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">
              <button data-id="{{ $row->batch_id }}" type="button" class="btn btn-icon btn-success rounded-pill lihat-data-berakhir" data-toggle="popover" title="Lihat Kandidat" data-content="Lihat Kandidat">
...
<div class="modal fade" id="lihatDataBerakhir" role="dialog" aria-labelledby="lihatDataBerakhirLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="lihatDataBerakhirLabel">Data Kandidat</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <table id="tableberakhir" class="table table-bordered table-hover" data-toggle="data-table">
          <thead>
            <tr class="text-center" style="color:#2d2d2d;">
              <th>No</th>
              <th>Nama</th>
...
<script>
  $(document).ready(function() {
    ...
    $('.lihat-data-berakhir').click(function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          console.log(data)
          var table = $('#tableberakhir').DataTable();
          table.clear();
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            rowHtml += '<td>';
            if (rowData.cv_file) {
              rowHtml += '<a href="{{ asset("storage/cv_files/") }}/' + rowData.cv_file + '" target="_BLANK" class="btn btn-icon btn-success rounded-pill lihat-data-berakhir" data-toggle="popover" title="Download CV" data-content="Download CV">' + IconDownload + ' CV</a> &nbsp;';
            } else {
              rowHtml += '';
            }
            if (rowData.portfolio_file) {
              rowHtml += '<a href="{{ asset("storage/portfolio_files/") }}/' + rowData.portfolio_file + '" target="_BLANK" class="btn btn-icon btn-success rounded-pill lihat-data-berakhir" data-toggle="popover" title="Download CV" data-content="Download CV">' + IconDownload + ' CV</a> &nbsp;';
            } else {
              rowHtml += '';
            }
            rowHtml += '</td>';
